...
def getDate(startdate=(2019, 12, 1)):
    def x(x):
        t = x.split("-")
        if len(t) < 3:
            return -1
        ...
        d2 = date(startdate[0], startdate[1], startdate[2])
        return (d - d2).days
...
def readFasta(path, to_upper=True, remove="whitespaces", truclength=None, checkseq=re3):
